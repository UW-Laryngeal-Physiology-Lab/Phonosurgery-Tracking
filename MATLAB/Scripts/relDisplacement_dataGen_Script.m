% relDisplacement_dataGen_Script.m
...
switch(nameConvention)
    ...
    case('new');
        lVid = mmreader(['inst' suffix '_left.avi']); 
        lBack = mmreader(['bgnd' suffix '_left.avi']);
        rVid = mmreader(['inst' suffix '_right.avi']); 
        rBack = mmreader(['bgnd' suffix '_right.avi']);
end
...
frames = [1,3000]; numFrames = 1 + (frames(2)-frames(1));
...
figure();
subplot(3,1,1);plot(XL1_start(1,:)); title('X Position (mm)');
subplot(3,1,2);plot(XL1_start(2,:)); title('Y Position (mm)');
subplot(3,1,3);plot(XL1_start(3,:)); title('Z Position (mm)');
%subplot(2,1,2); plot3(XR(1,:),XR(2,:),XR(3,:));

%% Compute Relative Displacement
integrateDist = sqrt(sum(cumsum(XL1_end-XL1_start,2).^2,1));
geomDist = sqrt(sum((XL1_start - ...
                     repmat(XL1_start(:,1),1,size(XL1_start,2))).^2,1));

fprintf('Integrated Displacement: %1.6f\n',integrateDist(end));
fprintf('Geometric Displacement: %1.6f\n',geomDist(end));

figure();
subplot(2,1,1); plot(integrateDist); title('Integrate Distance (mm)');
subplot(2,1,2); plot(geomDist); title('Geometric Distance (mm)');
