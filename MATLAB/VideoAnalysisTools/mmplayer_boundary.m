classdef mmplayer_boundary < handle
    ...
    properties (Hidden = true)
        ...
        % Boundary Label File Menu
        huimtraceboundary;
        huimshowboundary;
        
        % Region Analysis File Menu
        huimsetregion;
        huimviewregion;
        huimregionhistogram;
    ...
    methods (Hidden = true)
        ...
        function obj = createGui(obj)
            ...
            huimboundary = uimenu(obj.hfig,...
                'Label','Boundary Label');
            
                obj.huimtraceboundary = uimenu(huimboundary,...
                    'Label','Trace Boundary',...
                    'Callback',@(src,event)traceBoundary(obj));
                
                obj.huimshowboundary = uimenu(huimboundary,...
                    'Label','Show Boundary',...
                    'Callback',@(src,event)showBoundary(obj));
                
            huimregionanalysis = uimenu(obj.hfig,...
                'Label','Region Analysis');
                
                obj.huimsetregion = uimenu(huimregionanalysis,...
                    'Label','Set Region',...
                    'Callback',@(src,event)setRegion(obj));
                
                obj.huimviewregion = uimenu(huimregionanalysis,...
                    'Label','View Region',...
                    'Callback',@(src,event)viewRegion(obj));
                
                obj.huimregionhistogram = uimenu(huimregionanalysis,...
                    'Label','Region Histogram',...
                    'Callback',@(src,event)regionHistogram(obj));
        ...
        function setStatusTime(obj, time)
            str = sprintf('%s / %s', mmplayer_boundary.time2str(time), obj.durationStr);
    ...
    methods
        %% constructor
        function obj = mmplayer_boundary(mmreaderObj)
            ...
            obj.durationStr = mmplayer_boundary.time2str(obj.duration);            
            
            obj.repeat = true;
            obj.verbose = false;
            
            obj.boundaryData = cell(obj.numberOfFrames,1);
        ...
        %% Boundary Labeling
        function traceBoundary(obj)
            % Let User Draw Boundary Polygon
            h_poly = impoly(obj.haxes,'Closed',0);
            position = wait(h_poly);
            delete(h_poly);
            
            obj.boundaryData{obj.currentFrameIndex,1} = position;
        end
        
        function showBoundary(obj)
            % Display Polygon if Boundary Already Exists
            prevPosition = obj.boundaryData{obj.currentFrameIndex,1};
            
            if (~isempty(prevPosition))
                h_poly = impoly(obj.haxes,prevPosition,'Closed',0);
                position = wait(h_poly);
                delete(h_poly);
        ...
        function setRegion(obj)
            % Set Region Using Polygon Interactive Tool
            h_poly = impoly(obj.haxes,'Closed',1);
            obj.regionPosition = wait(h_poly);
            obj.regionMask = h_poly.createMask;
            delete(h_poly);
        end
        
        function viewRegion(obj)
            % Get Bounding Box
            xMin = floor(min(obj.regionPosition(:,1)));
            xMax = ceil(max(obj.regionPosition(:,1)));
            yMin = floor(min(obj.regionPosition(:,2)));
            yMax = ceil(max(obj.regionPosition(:,2)));
            
            % Display Masked Region
            regionIm = uint8(...
                repmat(obj.regionMask(yMin:yMax,xMin:xMax),[1,1,size(obj.currentFrame,3)]))...
                .* obj.currentFrame(yMin:yMax,xMin:xMax,:);
            figure();imshow(regionIm,'Border','Tight');
        end
        
        function regionHistogram(obj)
            % Grayscale Histogram
            if (size(obj.currentFrame,3) == 3)
                grayFrame = rgb2gray(obj.currentFrame);
                [counts,x] = imhist(grayFrame(obj.regionMask));
                figure(); stem(x,counts/sum(counts),'Marker','None');
                xlabel('Counts');ylabel('Intensity');
            elseif (size(obj.currentFrame,1) == 1)
                [counts,x] = imhist(obj.currentFrame(obj.regionMask));
                figure(); stem(x,counts/sum(counts),'Marker','None');
                xlabel('Counts');ylabel('Intensity');
            end
        end
