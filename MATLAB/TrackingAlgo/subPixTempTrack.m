function [tCorner,nCorner,subTCorner,wStats,ais] = subPixTempTrack(vidObj,...
    frames,templateSize,neighborhoodSize,varargin)
% SUBPIXTEMPTRACK Tracks Template Region in Video @ subpixel resolution
%
...
% and NCORNER are at pixel resolution.  SUBT is the subpixel resolution match of
% the template.  WSTATS is a structure with fields wSize, nx, and ny
% representing the template window's size and the neighborhood window side
% lengths (nx = ny).  The 'Update' property determines how the template is
% updated after a best match has been found, the 'Color' property
% determines whether to convert video frames to grayscale, and the
...
%        findTempMatch(nextFrame,temp,tCurr,nCurr,nSize,'ncc');
    try
        [tNext,nNext,score,fitMSE] = ...
            findTempMatch_sub(nextFrame,temp,tCurr,nCurr,nSize,'ncc');
    catch ME
