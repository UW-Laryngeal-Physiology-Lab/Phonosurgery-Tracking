function [tempNew,neighbNew,maxScore,simSurf] = ...
                 findTempMatch(frameIm,template,tCurr,nCurr,nSize,varargin)
% FINDTEMPMATCH Finds best match to template in frame neighborhood
%
% [TEMPNEW,NEIGHBNEW,MAXSCORE,SIMSURF] = ...
%           findTempMatch(FRAMEIM,TEMPLATE,TCURR,NCURR,NSIZE,MEASURE='ncc')
...
switch sim
    case 'ncc'
        ...
        [maxScore,maxDex] = max(nccScore(:));
        if(nargout == 4); simSurf = nccScore; end;
    case 'ssd'
        ssdScore = nMask .* ssdScore;
        [maxScore,maxDex] = max(ssdScore(:));
        if(nargout == 4); simSurf = ssdScore; end;
end
